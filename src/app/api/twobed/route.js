...
import TwoBed from "../../../../models/twobed";

export async function POST(request) {
  const { name, description, price, imageUrl, roomType } = await request.json();
  await connectMongoDB();
  await TwoBed.create({ name, description, price, imageUrl, roomType });
...
export async function GET() {
  await connectMongoDB();
  const twobeds = await TwoBed.find();
  ...
  const adjustedRooms = twobeds.map((twobed) => {
    let priceNumber = parseFloat(twobed.price);
    ...
    twobed.price = priceNumber.toString();

    return twobed;
  });

  return NextResponse.json({ twobeds: adjustedRooms });
}
